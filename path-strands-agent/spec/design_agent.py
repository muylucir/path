...
class DesignAgent:
    ...
    def __init__(self):
        ...
        skills_plugin = AgentSkills(skills=[os.path.join(skills_dir, "agent-patterns")])
...
|------------|------|-------|--------|------------|
| [Agent1] | [역할 — 구체적 책임 나열] | [입력 데이터 상세] | [출력 데이터 상세] | [High/Medium/Low — 근거 포함] |

**중요: 테이블에 HTML 태그 금지. 각 셀에 구체적 데이터 흐름과 복잡도 근거를 기술하세요.**

#### 2.2.2 Agent별 상세 설계

각 Agent에 대해 아래 형식으로 상세 설계를 작성하세요:

**[번호] [Agent Name]**

- **책임**: [에이전트의 구체적 책임과 역할 범위 — 2-3문장]
- **인지 패턴**: [사용하는 LLM Workflow 조합, 예: RAG + ReAct + Memory]
- **도구 목록**:
  - `tool_name` — [도구 설명]
  - (필요한 도구를 모두 나열)
- **판단 로직**:
  - [단계별 처리 흐름을 구체적으로 기술]
  - [조건 분기, 임계값, 규칙 기반 로직 포함]
  - [실패 시 fallback 경로 포함]
...
| 필드 | 작성 주체 | 소비 주체 | 설명 |
|------|-----------|-----------|------|
| [field_name] | [작성 Agent] | [소비 Agent 목록] | [데이터 설명] |
...
| 필드 | 저장소 유형 | 용도 |
|------|-------------|------|
| [field_name] | [벡터 DB / 관계형 DB / 등] | [용도 설명] |

### 2.4 Error Handling & Fallback Strategy

각 Agent 및 외부 연동 지점에서 발생 가능한 오류 시나리오와 복구 전략을 구체적으로 기술하세요.

| 오류 시나리오 | 감지 조건 | 복구 전략 | 최종 Fallback |
|---------------|-----------|-----------|---------------|
| [구체적 오류 상황] | [오류를 감지하는 조건/임계값] | [1차 복구 방법 — 재시도, 대체 경로 등] | [복구 실패 시 최종 대응 — 에스컬레이션, 안전 우선 처리 등] |

**작성 기준:**
- Agent 내부 로직 실패 (추론 실패, 검증 실패 등)
- 외부 API/DB 장애 (타임아웃, 오류 응답 등)
- Agent 간 데이터 정합성 오류 (이전 단계 결과와 불일치 등)
- 오케스트레이터 자체 장애 (파이프라인 중단, 체크포인트 복구 등)
- 각 시나리오별 재시도 횟수, 백오프 전략, 안전 우선 원칙을 명시하세요
"""
