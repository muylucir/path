...
def render_selected_data_sources_xml(items) -> str:
    """선택된 데이터 소스 목록을 LLM 프롬프트용 XML 블록으로 렌더.

    items: path-web의 StructuredDataSourceEntry 배열 (list of dict)
      필드: id, name, category, description, tags, auth_mode, has_secret_ref,
           access_pattern(optional), latency_tier(optional), schema_hint(optional),
           config_summary

    비어 있으면 <selected_data_sources_empty .../>를 반환해 LLM이 명확히 인지.
    민감값(secret_ref 자체 등)은 실지 않는다.
    """
    if not items:
        return (
            '<selected_data_sources_empty '
            'note="사용자가 카탈로그에서 선택하지 않음. additionalSources 자연어만 참고" />'
        )

    lines = [f'<selected_data_sources count="{len(items)}">']
    for it in items:
        if not isinstance(it, dict):
            continue
        attrs = [
            f'id="{_xml_attr(it.get("id"))}"',
            f'name="{_xml_attr(_sanitize(it.get("name") or ""))}"',
            f'category="{_xml_attr(it.get("category"))}"',
            f'auth="{_xml_attr(it.get("auth_mode"))}"',
            f'secret_ready="{"true" if it.get("has_secret_ref") else "false"}"',
        ]
        if it.get("access_pattern"):
            attrs.append(f'access="{_xml_attr(it["access_pattern"])}"')
        if it.get("latency_tier"):
            attrs.append(f'latency="{_xml_attr(it["latency_tier"])}"')
        lines.append(f"  <ds {' '.join(attrs)}>")
        desc = _sanitize(it.get("description") or "").strip()
        if desc:
            lines.append(f"    <description>{desc}</description>")
        cfg = _sanitize(it.get("config_summary") or "").strip()
        if cfg:
            lines.append(f"    <config_summary>{cfg}</config_summary>")
        hint = _sanitize(it.get("schema_hint") or "").strip()
        if hint:
            lines.append(f"    <schema_hint>{hint}</schema_hint>")
        tags = it.get("tags") or []
        if tags:
            tag_str = ",".join(_sanitize(str(t)) for t in tags if t)
            if tag_str:
                lines.append(f"    <tags>{tag_str}</tags>")
        lines.append("  </ds>")
    lines.append("</selected_data_sources>")
    return "\n".join(lines)
...
def get_feasibility_evaluation_prompt(
    form_data: dict,
    selected_data_sources: Optional[list] = None,
) -> str:
    """Step2: Feasibility 평가 프롬프트 생성

    selected_data_sources: path-web이 resolve한 StructuredDataSourceEntry 배열(dict 목록).
      존재하면 자연어 `additionalSources` 대신 1급 입력으로 XML 블록으로 주입.
      비어있거나 None이면 자연어 서술만 참고.
    """
...
데이터소스(자연어 보조): {_sanitize(data_source_str)}
...
- `<selected_data_sources>` 블록은 사용자가 카탈로그에 사전 등록하고 이 Agent용으로 선택한 리소스입니다. 자연어 '데이터소스' 보조 설명보다 이 블록을 우선 신뢰하세요.
- `data_access` 점수: 구조화된 연결정보(config_summary, auth, secret_ready, access_pattern)가 준비될수록 가점. `auth != "none"`인데 `secret_ready="false"`면 인증 준비 미흡으로 감점.
- `integration` 점수: 표준 SDK/프로토콜(S3, MCP)은 난이도 낮음, SaaS(GoogleSheets, Notion, Confluence)는 OAuth/rate-limit로 난이도 상대적으로 높음 — 단, pain_point 맥락이 우선.
- 선택된 DS가 없으면 `<selected_data_sources_empty .../>`입니다. 이때 `data_access`/`integration`은 자연어 설명만으로 평가하되 confidence를 high가 아닌 값으로 낮추세요.
- **evidence_ds_ids**: 각 breakdown 항목과 weak_item에 대해, 판단 근거가 된 `<ds id="...">` 값을 `evidence_ds_ids` 배열에 정확히 기입하세요. 선택된 DS와 무관한 항목은 빈 배열.
</data_source_usage_rules>
...
{{
  "feasibility_breakdown": {{
    "data_access": {{
      ...
      "evidence_ds_ids": ["해당 항목 판단의 근거가 된 ds.id 값들 (없으면 빈 배열)"]
    }},
    "decision_clarity": {{
      "score": 7,
      "confidence": "high/medium/low",
  ...
  "weak_items": [
    {{
      ...
      "evidence_ds_ids": ["이 weak item이 지적하는 선택된 DS id 목록 (없으면 빈 배열)"]
    }}
  ],
...
| 축 | 무엇을 보는가 | 0 | 1 | 2 |
|---|---|---|---|---|
| 축1: 도구 복잡도 | tool_count × domain_count ÷ 10 | <1.5 | 1.5~3.0 | >3.0 |
| 축2: 역할 분리도 | 상충하는 페르소나 쌍 수 | 0쌍 | 1쌍(분리 가능) | 2+쌍 |
| 축3: 흐름 복잡도 | 분기/루프/병렬/동적라우팅 | 선형~분기1 | 분기2-3 or 루프1 | 분기4+/루프2+/병렬/동적 |

### 합산 판정
| 합산 | 판정 | 설계 방향 |
|:---:|:---:|---|
| 0-1 | 🔵 싱글 | Layer 2 LLM Workflow만으로 충분. 멀티 에이전트 오버헤드가 이점보다 큼 |
| 2-3 | 경계 | 양쪽 모두 가능. 아래 경계 기준 4가지로 결정 (2+가 멀티 열이면 → 멀티) |
| 4-6 | 🟣 멀티 | 싱글로는 컨텍스트/품질/복잡도 한계에 도달할 가능성 높음 |

### 경계 영역 (합산 2-3) 판단
| 조건 | → 🔵 싱글 | → 🟣 멀티 |
|---|:---:|:---:|
| 상충 쌍이 Prompt Chaining으로 분리 가능? | 가능 | 불가능 |
| 파이프라인이 고정 순서? | 고정 | 동적 |
| 실시간 응답 필요? | 필요 | 불필요(배치 OK) |
| 개별 단계 실패 시 독립 재시도 필요? | 불필요 | 필요 |

### 멀티 에이전트 패턴 선택 (가장 높은 축으로)
| 최고 축 | 권장 패턴 | 이유 |
|:---:|---|---|
| 축1 (도구) | **Agents as Tools** | 도메인별 전문 에이전트로 분리 |
| 축2 (역할) | **Swarm** | 역할별 에이전트 핸드오프 (생성↔비평 등) |
| 축3 (흐름) | **Graph** | 조건부 엣지로 복잡한 흐름 제어 |
| 동일 | **Workflow** (고정 순서) 또는 **Graph** (동적 순서) | 종합 복잡도 |

**중요**: 대화 중 3축 점수를 사용자에게 직접 노출하지 마세요. 내부 판단 근거로만 활용하고, 사용자에게는 자연어로 근거를 설명하세요.
</architecture_guide>
...
| Agentic Workflow | 구조 | 3축 합산 판정 |
|-----------------|------|:----------:|
| 🔵 **싱글 에이전트** | 단일 Agent + Tools | 합산 0-1 |
| 🟣 **Agents as Tools** | Orchestrator → 전문 Sub-agents | 합산 4+ (축1 최고) |
| 🟣 **Swarm** | 동등한 에이전트 간 핸드오프 | 합산 4+ (축2 최고) |
| 🟣 **Graph** | 방향성 그래프로 흐름 정의 | 합산 4+ (축3 최고) |
| 🟣 **Workflow** | 사전 정의된 순차 파이프라인 | 합산 4+ (축 동일) |

### 3계층 조합 예시

| 사례 | Layer 1 | Layer 2 | Layer 3 | 3축 점수 |
|------|---------|---------|---------|:--------:|
| 고객지원 봇 | RAG + Tool-based | ReAct + Routing | 🔵 싱글 | 1+0+1=2 |
| 코드 리뷰 시스템 | Coding + Memory | Reflection + Planning | 🟣 Agents as Tools | 2+2+1=5 |
| 연구 보고서 생성 | RAG + Tool-based | Prompt Chaining | 🟣 Agents as Tools | 2+2+2=6 |
| 콘텐츠 생성 파이프라인 | Memory + Tool-based | Reflection | 🟣 Swarm | 1+2+1=4 |
| 계약서 검토 자동화 | RAG + Memory | Prompt Chaining + Reflection | 🟣 Graph | 2+2+2=6 |
...
def get_pattern_analysis_prompt(
    form_data: dict,
    feasibility: dict,
    improvement_plans: Optional[dict] = None,
    selected_data_sources: Optional[list] = None,
) -> str:
...
<data_source_usage_rules>
- `<selected_data_sources>` 블록은 사용자가 사전 등록·선택한 실제 리소스입니다. 자연어 데이터소스 설명보다 이 블록을 우선 신뢰하세요.
- 패턴 추천은 pain_point와 process_steps를 기준으로 하되, 선택된 DS의 category/access/latency가 패턴 실행 가능성에 주는 함의(예: RAG 리소스 존재, read-write RDBMS 포함)를 근거로 활용하세요.
- 사용자가 이미 제공한 DS 정보(이름·config·schema_hint·tags)를 다시 묻지 마세요. 질문은 패턴 결정에 아직 부족한 사항에 한정.
- 선택된 DS가 비어있으면 자연어 설명만으로 추론하되, 확신도를 과장하지 마세요.
</data_source_usage_rules>
...
**3축 점수 적용**: 첫 응답부터 사용자 입력에서 파악 가능한 도구 수, 도메인 수, 역할 상충, 흐름 특성을 기반으로 잠정 3축 점수를 내부적으로 추정하세요. 불확실한 축을 명확히 하기 위한 질문을 우선 생성하세요.
...
def get_pattern_chat_prompt(
    ...
    Args:
        ...
        selected_data_sources: 선택된 카탈로그 항목 (stateless fallback 경로에서 첫 턴에 주입)
    """
...
- 도구 사용 여부와 관계없이 사용자에게 바로 실질적인 답변만 제공하세요."""
    ...
    ds_xml = render_selected_data_sources_xml(selected_data_sources or [])
    ds_block = f"\n{ds_xml}\n" if selected_data_sources else ""

    if history_text is not None:
        # Stateless fallback: 전체 히스토리를 프롬프트에 포함
        return f"""{ds_block}{_sanitize(history_text)}

{instructions}"""
    else:
        # Stateful: SDK messages에 이미 히스토리 존재, 사용자 메시지만 전달
        return f"""{ds_block}{_sanitize(user_message)}

{instructions}"""


def get_pattern_finalize_prompt(
    form_data: dict,
    feasibility: dict,
    improvement_plans: Optional[dict] = None,
...
8. improved_feasibility를 반드시 계산하여 포함하세요 (null 금지). 계산 기준:
   ...
   - breakdown: 5개 항목별 original_score, improved_score(숫자), improvement_reason"""
    ...
    output_schema = {
        ...
        "evidence_ds_ids": ["패턴 선택의 근거가 된 ds.id 목록 (없으면 빈 배열)"]
    }
...
{improvement_xml}
<architecture_guide>
아키텍처 판단: 3축 점수 평가 (대화에서 수집한 정보 기반)
- 축1 도구 복잡도: tool_count × domain_count ÷ 10 → 0/1/2
- 축2 역할 분리도: 상충 페르소나 쌍 수 → 0/1/2
- 축3 흐름 복잡도: 분기/루프/병렬/동적라우팅 → 0/1/2
- 합산 0-1: 싱글, 2-3: 경계(4가지 추가 기준으로 결정), 4-6: 멀티
- 멀티 패턴: 최고 축 → 축1=Agents as Tools, 축2=Swarm, 축3=Graph, 동일=Workflow/Graph
three_axis_scores 필드에 점수를, architecture_reason에 근거를 반드시 포함하세요.
</architecture_guide>
...
7. recommended_architecture는 3축 점수(도구 복잡도/역할 분리도/흐름 복잡도)를 계산하여 결정하세요. three_axis_scores 필드에 각 축 점수(0-2)와 합산(0-6), 근거를 반드시 포함하세요. architecture_reason에 "축1: X, 축2: Y, 축3: Z, 합산 N → 판정"을 명시하세요. 경계(합산 2-3)인 경우 4가지 추가 기준(Prompt Chaining 분리 가능성, 파이프라인 순서 고정성, 실시간 응답 필요성, 독립 재시도 필요성) 중 어느 것에 해당하는지 기술하세요.
8. pattern 필드는 반드시 "Layer1(패턴 + 패턴) + Layer2(패턴 + 패턴) + Layer3(패턴)" 형식으로 작성하세요. 각 Layer 내부 패턴은 +로 결합하고, 각 Layer는 Layer1()/Layer2()/Layer3() 접두사와 괄호로 감싸고, Layer 간은 +로 결합합니다. 예시: "Layer1(RAG + Tool-based) + Layer2(ReAct + Routing) + Layer3(Workflow)". 다른 구분자(x, ., /, 등) 사용 금지.
{improved_feasibility_rules}
</rules>
