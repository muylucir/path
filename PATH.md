...
싱글/멀티 에이전트 판단은 3개 축(각 0/1/2)의 합산 점수로 결정:

| 축 | 무엇을 보는가 | 0 | 1 | 2 |
|---|---|---|---|---|
| 도구 복잡도 | tool_count × domain_count ÷ 10 | <1.5 | 1.5~3.0 | >3.0 |
| 역할 분리도 | 상충 페르소나 쌍 수 | 0쌍 | 1쌍 | 2+쌍 |
| 흐름 복잡도 | 분기/루프/병렬/동적라우팅 | 선형~분기1 | 분기2-3 | 분기4+/병렬/동적 |

| 합산 | 판정 |
|:---:|:---:|
| 0-1 | 🔵 싱글 에이전트 |
| 2-3 | 경계 (4가지 추가 기준으로 결정) |
| 4-6 | 🟣 멀티 에이전트 |

### 멀티 에이전트 협업 패턴 (가장 높은 축으로 선택)

| 최고 축 | 패턴 | 설명 |
|:---:|------|------|
| 축1 (도구) | **Agents as Tools** | 도메인별 전문 에이전트로 분리 |
| 축2 (역할) | **Swarm** | 역할별 에이전트 핸드오프 |
| 축3 (흐름) | **Graph** | 조건부 엣지로 복잡한 흐름 제어 |
| 동일 | **Workflow** / **Graph** | 고정/동적 순서에 따라 선택 |
...
- 확정 시 JSON 형식의 최종 분석 결과 생성 (`three_axis_scores`, `updated_autonomy`, `automation_level` 포함)
...
| **보안** | Zod 스키마 검증, XML 태그 포괄 제거 + Unicode 방어 (`_sanitize`), safe_file_read 경로 제한 (realpath + 경계 검증), Payload 크기 제한, 세션 TTL/maxsize, 에러 메시지 generic화, Cognito 인증 |
